...
5. to amend initiation of reproducibility checks process via dropbox file request link (see below) - starting from templated _conditional acceptance_ message of managing editor - see proposition of new template below.
:::

## New Replication Checks Process Starting with Conditionally Accepted Papers

1. `ME` sends _conditional acceptance_ message.
2. `ME` message contains following info:
    1. Authors must submit paper, appendix and signed checklist ([https://bit.ly/3Qa0HW5](https://bit.ly/3Qa0HW5) is out of sync) via `EE`. After submission on EE, they must submit a replication package via dropbox link sent by DE, where package is structured as in https://ejdataeditor.github.io/.
...
[https://bit.ly/3ivH38d](https://bit.ly/3ivH38d)
...
1. After conditional acceptance by the `ME`, `EO` waits on `HT` of `EE` until the required zip file arrives. See above for what the required contents are. (*paper, appendices and checklist*). If contents are missing, cycle back to authors until correct and signed.
1. After successful arrival on `EE`, `EO` logs the paper's metadata in our shared googlesheet `SS`, filling all fields as shown in figure @fig-logmetadata (leaving the rest empty). Upon turning blue, the googlesheet will send an automated message to the Data Editor ({{< var de.first >}}) - no further action on behalf of `EO` required.
...
{{< var de.first >}} receives an automated message that a new paper has arrived to be checked for reproducibility. He creates a new folder inside the shared dropbox folder `EJ-2-submitted-replication-packages` (`EctJ-2-submitted-replication-packages` for `EctJ`) named with the main author's last name and their MS number, and *sends a file request link to the author via email*.
...
## Step 7. Editorial Office: Request `Zenodo` Upload

`EO` sends an email to the authors via `EE` (copying {{< var de.first >}} and {{< var pm.first >}}) with a request to upload the package to the [`EJ` community at Zenodo](https://zenodo.org/communities/ej-replication-repository), as indicated on the [DE's website](https://ejdataeditor.github.io/package). 
