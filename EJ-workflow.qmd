...
{{< var eo.first >}} handles the first two types, {{< var de.first >}} downloads the third type, and leaves a note on `EE` as shown in figure @fig-acqui. Notice that this happens *after the article has been assigned to replicators*!
...
Either {{< var eo.first >}} or {{< var de.first >}} places the replication package into folder `2. Submitted replication packages` of the shared dropbox `DB`, which has the following layout:
