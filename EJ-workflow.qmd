...
v{{< var version >}}: Proposition for unique EJ + EctJ workflow by Florian
...
1. the exact same wording of data policy for EJ and EctJ. I don't see any particular reason why they should differ.
2. the exact same process of reproducibility checks for both journals.
3. to keep two different dropboxes to collect packages.
3. to keep two separate email addresses `ejdataeditor@gmail.com` or `ectjdataeditor@gmail.com`
3. to handle all communications via those emails instead of `EE`. I can upload histories to `EE` correspondance folder upon request.
3. to have a _unique_ RES Data Editor website instead of 2 almost identical ones for each journal. There is a small change to make in the zenodo upload section, where we need to say _if you publish with EJ, go to the EJ community, if you publish with EctJ, go to the EctJ community_
4. to have a unique google spreadsheet to log metadata (adding a column `journal` with values `EJ` or `EctJ` to differentiate)
5. to amend initiation of reproducibility checks process via dropbox file request link (see below) - starting from templated _conditional acceptance_ message of managing editor which automatically copies `ejdataeditor@gmail.com` or `ectjdataeditor@gmail.com` - I will initiate the process upon receiving the _conditional acceptance_ of the ME - see proposition of new template below.
:::

## New Replication Checks Process Starting with Conditionally Accepted Papers

1. `ME` sends _conditional acceptance_ message, and copies `DE`'s email.
2. `ME` message contains following info:
    1. Authors must submit paper, appendix and signed checklist via `EE`. They must also submit a replication package via dropbox link sent by `DE`.
    1. Paper and appendix via `EE` named in format `MSXXXXXXXX-main-YYYYMMDD.tex`, `MSXXXXXXXX-appendix-YYYYMMDD.tex`, `MSXXXXXXXX-main-YYYYMMDD.pdf`, `MSXXXXXXXX-appendix-YYYYMMDD.pdf`.
    adhering to certain formatting rules: [https://bit.ly/3g2YBaB](https://bit.ly/3g2YBaB), and following the guidelines ([https://bit.ly/3ivH38d](https://bit.ly/3ivH38d) is out of sync). Having those files on `EE` serves the purpose of having a master version of the accepted paper on record, against we can check the paper after potentially several rounds of replication checks.
    2. Checklist needs to be signed ([https://bit.ly/3Qa0HW5](https://bit.ly/3Qa0HW5) is out of sync) and submitted to `EE`.
    3. Authors will be invited by the `DE` to submit their replication package via dropbox file request link. The package needs to be structured as outlined on [https://ejdataeditor.github.io/](https://ejdataeditor.github.io/) (or new website resdataeditor.githb.io as the case may be)
...
2. Update conditional acceptance template messages of all `ME`s
3. Do exactly the same for `EctJ` `ME`s.


## Draft of Conditional Acceptance Message Template

Dear Author:

I am pleased to inform you that I have decided to "conditionally accept" your submission, MS 20220160, titled
...
As a next step, I am asking you to submit the final version of all your files in two separate ways: Your paper via Editorial Express, and your replication package via dropbox file request link (see below).

### Submission of Paper via Editorial Express

On Editorial Express, you should submit a single zip file containing three objects: two directories and one pdf file, as follows.
...
In preparing the final version of your paper, please consult the instructions for authors at:
https://bit.ly/3ivH38d
to make sure your final submission conforms to the journal's requirements.

For creating manuscripts in LaTex, The Economic Journal strongly recommends the use of its TeX template which is downloadable at: https://bit.ly/3g2YBaB

### Submission of Replication Package via dropbox link
...
You will shortly receive a file request link from our Data Editor, where you will be able to upload the package in the required format, as soon as your are ready.
